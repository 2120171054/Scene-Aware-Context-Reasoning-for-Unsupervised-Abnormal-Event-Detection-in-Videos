...
def filter_object_boxes(data, heights, widths, area_frac_thresh):
    ...
    for i, img in enumerate(data):
        ...
        for obj in img['objects']:
            if float(obj['h'] * obj['w']) > area * area_frac_thresh: # should be filterd during tracking
...
def main(args):
    ...
    obj_data = filter_by_idx(obj_data, valid_im_idx)# filter courppted imgs
    ...
    num_o=0
    for i in obj_data:
        num_o=num_o+len(i['objects'])
    print(num_o)
    # sync objects from rel to obj_data: object and subject in relations are added in object
    ...
    ######### random labels
    object_label_ = np.random.randint(1, 11, (encoded_label.shape[0],1))

    #########
    # f.create_dataset('labels', data=encoded_label)
    f.create_dataset('labels', data=object_label_)
    for k, boxes in encoded_boxes.items():
    ...
    ######### random predicates
    encoded_predicate_ = np.random.randint(1, 11, (encoded_predicate.shape[0],1))
    
    f.create_dataset('predicates', data=encoded_predicate_)
   ...
   # build train/val/test splits
    ...
    if args.mode=='train':
        ...
        split = 2 * np.ones(len(obj_data), dtype=np.int32)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imdb', default='../avenue/avenue_train_imdb_1800.h5', type=str)
    parser.add_argument('--object_input', default='../avenue/venue_video_train_objects.json', type=str)
    parser.add_argument('--mode', default='train', type=str)
    parser.add_argument('--relationship_input', default='../avenue/avenue_video_train_relationships.json', type=str)
    parser.add_argument('--metadata_input', default='../avenue/avenue_video_train_image_data.json', type=str)
    ...
    parser.add_argument('--num_objects', default=10, type=int, help="set to 0 to disable filtering")
    parser.add_argument('--num_predicates', default=10, type=int, help="set to 0 to disable filtering")
    parser.add_argument('--min_box_area_frac', default=0.0002, type=float)
    parser.add_argument('--json_file', default='avenue_train_-SGG-dicts.json')
    parser.add_argument('--h5_file', default='avenue_train_-SGG.h5')
    parser.add_argument('--load_frac', default=1, type=float)
    parser.add_argument('--use_input_split', default=False, type=bool)
    parser.add_argument('--train_frac', default=0, type=float)
    parser.add_argument('--val_frac', default=0, type=float)
