...
from networks.factory_ex import get_network
...
def im_detect(sess, net, inputs, im, boxes, bbox_reg, multi_iter):
    ...
    for act_i in range(10):
        curr_box = boxes[act_i*num_box_per_image:(act_i+1)*num_box_per_image]
        for i in range(num_box_per_image):
            for j in range(num_box_per_image):
                ...
                x2=curr_box[j][0]
                y2=curr_box[j][1]
                if i < j and  math.sqrt((x1-x2)**2+(y1-y2)**2)<im_size_min/2 and i!=num_box_per_image-1 and j!=num_box_per_image-1:
                    relations.append([i+act_i*num_box_per_image, j+act_i*num_box_per_image])
    if len(relations)==0:
...
def gt_rois(roidb): # no use of max_overlaps
    overlaps = roidb['max_overlaps']
    gt_inds = np.where(overlaps == 1)[0]
...
def extract_net(net_name, weight_name, imdb, mode, max_per_image=100):
    ...
    ## 
    vert_features_file=h5py.File(os.path.join(cfg.VG_DIR, 'avenue_vert_feature.h5'), 'w')
    
    vert_feature = vert_features_file.create_dataset('vert_cls', (num_rois,512), dtype=np.float32)
    ...
    for im_i in xrange(num_images):
        ...
        for mode in eval_modes:
            ...
            for iter_n in multi_iter:
                ...
                print(im_i,im_to_first_box,im_to_last_box,sg_entry['vert'].shape,sg_entry['scores'].shape)
                record_vert_features=sg_entry['vert']
                vert_feature[im_to_first_box:im_to_last_box+1,:]=record_vert_features[0:im_to_last_box+1-im_to_first_box,:]
